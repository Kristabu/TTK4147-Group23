...
void main()
{
	Array my_array = array_new(3);
	for(int i = 0; i<17; i++)
	{
		if(i>my_array.capacity)
			array_reserve(&my_array, my_array.capacity * 2);
		array_insertBack(&my_array, i);
		printf("capacity");
		_array_printSingleLongHelper(my_array.capacity);
		printf("\n");
	}
}
